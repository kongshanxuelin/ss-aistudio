...
class AnnotatedCursor(Cursor):
    ...
    **cursorargs : `matplotlib.widgets.Cursor` properties
    ...
    def __init__(self, xticks, mapLine, numberformat="{0:.4g};{1:.4g}", offset=(5, 5),
                 dataaxis='x', textprops={}, **cursorargs):
        self.xticks = xticks
        # The line object, for which the coordinates are displayed
        self.mapLine = mapLine        #key:gjkCode, value:[]
        ...
        self.set_xposition(0)
        # Create invisible animated text
        self.text = self.ax.text(
            self.ax.get_xbound()[0],
            self.ax.get_ybound()[0],
            "null",
            bbox=dict(facecolor="y", alpha=0.2),
            multialignment="left",
    ...
    def set_disable(self, disabled):
        ...
        # super().set_visible(not disabled)
    ...
    def onmove(self, event):
        ...
        if event.inaxes != self.ax:
            # print(f"event.inaxes != self.ax, event.inaxes:{event.inaxes}, self.ax:{self.ax}")
            self.lastdrawnplotpoint = None
        ...
        if event.xdata is not None and event.ydata is not None:
            ...
            plotpoint = self.set_xposition(event.xdata)
        ...
        if plotpoint is not None:
            ...
            posText = [event.xdata, event.ydata]
            posText = self.ax.transData.transform(posText)
            posTextTmp = posText
            # print(f"posTextTmp: ({posTextTmp})")
            posText = posText + self.offset
            posText = self.ax.transData.inverted().transform(posText)
            self.text.set_position(posText)
            
            date = datetime.datetime.strftime(datetime.datetime.fromtimestamp(plotpoint[0]), '%Y-%m-%d')
            tempText = date
            for gjkCode, line in self.mapLine.items():
                xdata = line.get_xdata()
                ydata = line.get_ydata()
                assert(len(xdata) == len(ydata))
                vec = np.where(xdata == plotpoint[0])
                index = None
                if len(vec) > 0:
                    index = vec[0]
            ...
            self.text.set_text(tempText)
            self.text.set_visible(self.visible)

            text_bbox = self.text.get_tightbbox()
            ax_bbox = self.ax.get_window_extent()
            # width = text_bbox.width
            # height = text_bbox.height
            # print(f"{text_bbox.width} + {event.x}")

            if text_bbox.width + (posTextTmp[0] - ax_bbox.x0) > ax_bbox.width:
                # print(f"{text_bbox.width} + {event.x - ax_bbox.x0} > {ax_bbox.width}")
    ...
    def set_xposition(self, xpos):
        lim = self.ax.get_xlim()
        if xpos is not None and lim[0] <= xpos <= lim[-1]:
            # Find closest x value in sorted x vector.
            # This requires the plotted data to be sorted.
            index = np.searchsorted(self.xticks, xpos)
            # Return none, if this index is out of range.
            if index < 0 or index >= len(self.xticks):
                return None
            # Return plot point as tuple.
            return (self.xticks[index], 0)
